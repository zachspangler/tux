...
try {
	...
	if($method === "POST") {
		...
		$profileName = ($requestObject->profileFirstName . " " . $requestObject->profileLastName);

		//create the profile object and prepare to insert into the database
		$profile = new Profile($profileId, $profileActivationToken, $requestObject->profileEmail, $profileHash, $profileName, $profilePhone, $profileSalt);

		//insert the profile into the database
		$profile->insert($pdo);

		//compose the email message to send with the activation token
		$messageSubject = "One more step before you can start using Tux, just confirm your account through your email.";
		...
		$message = <<< EOF
<h2>Welcome to Tux.</h2>
<p>In order to start using Tux, you must confirm your account.</p>
...
EOF;
		...
		// update reply
		$reply->message = "Thank you for creating a Tux profile. One more step before you can start using Tux. We have sent you an email to confirm your account, please open the email and click the link to confirm.";
	} else {
